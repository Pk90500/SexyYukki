...
from YukkiMusic.utils.database.mongodatabase import get_channel_top, update_channel_top
        ...
        try:
            for chat_id in chatstats:
                for dic in chatstats[chat_id]:
                    ...
                    print(f"c_id = {c_id}")
                    if not c_id is None:
                        spot = await get_channel_top(chat_id, c_id)
                        if spot:
                            spot = spot["spot"]
                            next_spot = spot + 1
                            new_spot = {"spot": next_spot, "title": "j"}
                            await update_channel_top(chat_id, c_id, new_spot)
                        else:
                            next_spot = 1
                            new_spot = {"spot": next_spot, "title": "j"}
                            await update_channel_top(chat_id, c_id, new_spot)
