...
async def put_queue(
    ...
    if vidid != "telegram":
        try:
            videosSearch = VideosSearch(vidid, limit = 1)
            videosResult = await videosSearch.next()
            c_id = (videosResult["result"][0]["channel"]["id"])
            to_append = {"vidid": vidid, "cid": c_id, "title": title}
        except:
            to_append = {"vidid": vidid, "cid": None, "title": title}
    else:
        to_append = {"vidid": vidid, "cid": None, "title": title}
