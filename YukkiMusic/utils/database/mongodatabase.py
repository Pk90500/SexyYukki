...
channeltopdb = mongodb.channeltopdata


async def get_channel_toplist() -> dict:
    results = {}
    async for chat in channeltopdb.find({"chat_id": {"$lt": 0}}):
        for i in chat["vidid"]:
...
async def get_channels(chat_id: int) -> Dict[str, int]:
    ids = await channeltopdb.find_one({"chat_id": chat_id})
    if not ids:
        return {}
    return ids["vidid"]


async def get_channel_top(chat_id: int, name: str) -> Union[bool, dict]:
    ids = await get_channels(chat_id)
    if name in ids:
        return ids[name]


async def update_channel_top(chat_id: int, name: str, vidid: dict):
    ids = await get_channels(chat_id)
    ids[name] = vidid
    await channeltopdb.update_one(
